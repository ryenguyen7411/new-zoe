...
if ( !class_exists( 'Woo_Bulk_Discount_Plugin_t4m' ) ) {
	...
	class Woo_Bulk_Discount_Plugin_t4m {
		...
		public function filter_subtotal_price( $price, $values ) {
			...
			$new_css = esc_attr( get_option( 'woocommerce_t4m_css_new_price', 'color: #4AB915; font-weight: bold;' ) );
			$bulk_info = sprintf( __( '%s discount', 'wc_bulk_discount' ), ( get_option( 'woocommerce_t4m_discount_type', '' ) == 'flat' || get_option( 'woocommerce_t4m_discount_type', '' ) == 'fixed' ? get_woocommerce_currency_symbol() . $coeff : ( round( ( 1 - $coeff ) * 100, 2 ) . "%" ) ) );

			return "<span class='discount-info' title='$bulk_info'>" .
			"<span>$price</span>" .
			"<span class='new-price' style='$new_css'><br/>($bulk_info)</span></span>";
		...
		public function filter_subtotal_order_price( $price, $values, $order ) {
			...
			$new_css = esc_attr( get_option( 'woocommerce_t4m_css_new_price', 'color: #4AB915; font-weight: bold;' ) );
			$bulk_info = sprintf( __( '%s discount', 'wc_bulk_discount' ), ( $discount_type == 'flat' || $discount_type == 'fixed' ? get_woocommerce_currency_symbol() . $coeff : ( round( ( 1 - $coeff ) * 100, 2 ) . "%" ) ) );
...
jQuery(document).ready(function() {
  ...
  e.find('#add_discount_line<?php echo $i; ?>').click(function() {
    if (<?php echo $i; ?> == 1 || (e.find('#_bulkdiscount_quantity_<?php echo max($i-1, 1); ?>').val() != '' &&
        <?php if ( get_option( 'woocommerce_t4m_discount_type', '' ) == 'flat' ) : ?> e.find(
          '#_bulkdiscount_discount_flat_<?php echo max($i-1, 1); ?>').val() != ''
        <?php elseif ( get_option( 'woocommerce_t4m_discount_type', '' ) == 'fixed' ) : ?> e.find(
          '#_bulkdiscount_discount_fixed_<?php echo max($i-1, 1); ?>').val() != ''
        <?php else: ?> e.find('#_bulkdiscount_discount_<?php echo max($i-1, 1); ?>').val() != ''
        <?php endif; ?>
      )) {
